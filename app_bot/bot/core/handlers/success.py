...
_COMPRESS_LOG = logging.getLogger('VideoCompressor')
...
class SuccessDownloadHandler(AbstractDownloadHandler):
    ...
    async def _set_upload_message(self, media_object: BaseMedia) -> None:
        ...
        try:
            await self._bot.edit_message_text(
                ...
                text=f'⬆️ {bold(f"Uploading {media_object.current_filename}")}\n📏 {bold("Size")} {media_object.file_size_human()}\n[            ] 0.0%',
            )
    ...
    async def _handle_media_object(self, media_object: BaseMedia) -> None:
        try:
            ...
            if self._upload_is_enabled():
                ...
                except _OversizedVideoError:
                    await self._compress_and_validate(media_object)
                coros = (
    ...
    def _validate_file_size_for_upload(self, media_obj: BaseMedia) -> None:
        ...
        if _file_size > max_file_size:
            if isinstance(media_obj, Video):
                _COMPRESS_LOG.info(
                    'File "%s" (%s) exceeds limit (%s), compressing...',
                    media_obj.current_filename,
                    format_bytes(_file_size),
                    format_bytes(max_file_size),
                )
                raise _OversizedVideoError(max_file_size=max_file_size)
            err_msg = (
    ...
    async def _compress_and_validate(self, media_obj: Video) -> None:
        max_file_size = self._get_max_file_size()
        for attempt in range(3):
            crf = 28 + attempt * 4
            await self._compress_video(media_obj, crf)
            if media_obj.current_file_size() <= max_file_size:
                _COMPRESS_LOG.info(
                    'Compressed "%s" to %s (CRF %d)',
                    media_obj.current_filename,
                    media_obj.file_size_human(),
                    crf,
                )
                return
            _COMPRESS_LOG.warning(
                'Compressed "%s" still too large (%s) after attempt %d (CRF %d)',
                media_obj.current_filename,
                media_obj.file_size_human(),
                attempt + 1,
                crf,
        ...
        raise ValueError(
            f'Failed to compress "{media_obj.current_filename}" below '
            f'{format_bytes(max_file_size)} after 3 attempts'
        )

    async def _compress_video(self, media_obj: Video, crf: int) -> None:
        src = media_obj.current_filepath
        ...
        _COMPRESS_LOG.info('Running: %s', cmd)
        ...
        if proc.returncode != 0:
            ...
            raise RuntimeError(f'FFmpeg compression failed (CRF {crf}): {err}')
        if not dst.exists():
            raise RuntimeError(f'FFmpeg output file not found: {dst}')
        media_obj.mark_as_converted(filepath=dst)

    def _get_max_file_size(self) -> int:
        if self._body.context.source is TaskSource.API:
            return self._bot.conf.telegram.api.upload_video_max_file_size
        user = self._bot.allowed_users[self._get_sender_id()]
        return user.upload.upload_video_max_file_size


class _OversizedVideoError(Exception):
