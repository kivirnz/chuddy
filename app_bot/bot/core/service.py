...
class UrlService:
    ...
    async def process_urls(self, urls: list[URL]) -> None:
        ...
        for url in urls:
            if url.from_chat_id:
                ChatActionManager().add_download(url.from_chat_id)
            
            sent = await self._send_to_worker(url)
            
            if not sent and url.from_chat_id:
                ChatActionManager().remove_download(url.from_chat_id)
...
class UrlParser:
    ...
    def parse_urls(
        ...
        url_media_types: dict[str, 'yt_shared.enums.DownMediaType'] | None = None,
    ) -> list[URL]:
        message: Message = context['message']
        user: UserSchema = context['user']
        ack_message: Message = context['ack_message']
        from_user_id = message.from_user.id if message.from_user else None
        url_media_types = url_media_types or {}
        return [
            URL(
                url=url,
                ...
                message_id=context.get('reply_target_id', message.id),
                ack_message_id=ack_message.id,
                save_to_storage=user.save_to_storage,
                download_media_type=url_media_types.get(orig_url, user.download_media_type),
            )
