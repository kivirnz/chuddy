...
class MediaDownloader:
    ...
    def download(
        self, host_conf: AbstractHostConfig, media_payload: InbMediaPayload, loop: 'asyncio.AbstractEventLoop | None' = None
    ) -> DownMedia:
        try:
            return self._download(host_conf=host_conf, media_payload=media_payload, loop=loop)
        except Exception:
        ...
        with TemporaryDirectory(prefix='tmp_media_dir-', dir=tmp_down_path) as tmp_dir:
            ...
            def _progress_hook(d: dict) -> None:
                ...
                try:
                    ...
                    if _loop is None:
                        self._log.warning('Download progress: no event loop available')
                        return
                    
                    raw_filename = d.get('filename') or ''
                    clean_name = _Path(raw_filename).name if raw_filename else None
                    if clean_name and clean_name.endswith('.part'):
                        clean_name = clean_name[:-5]
                    
                    total = d.get('total_bytes') or d.get('total_bytes_estimate')
                    payload = ProgressPayload(
                        task_id=str(media_payload.id) if media_payload.id else None,
                        message_id=media_payload.message_id or 0,
                        ack_message_id=media_payload.ack_message_id,
                        from_chat_id=media_payload.from_chat_id,
                        status='Downloading',
                        progress_percent=d.get('_percent_str'),
                    ...
                    fut = asyncio.run_coroutine_threadsafe(
                        publisher.send_download_progress(payload), _loop
                    )
                    fut.add_done_callback(_on_progress_sent)
                except Exception:
                    self._log.warning('Failed to send download progress', exc_info=True)
            
            def _on_progress_sent(fut):
                try:
                    fut.result()
                except Exception as exc:
                    self._log.warning('RabbitMQ progress publish failed: %s', exc)
